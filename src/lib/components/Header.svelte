...
>
	<div class="container-custom flex h-24 items-center justify-between">
		...
		>
			...
			<span class="absolute bottom-0 left-0 w-0 h-[2px] bg-[#ffde21] group-hover:w-full transition-all duration-500 ease-in-out"></span>
			<!-- Subtle glow effect on hover -->
			<span class="absolute inset-0 -z-10 opacity-0 blur-lg bg-[#ffde21]/20 group-hover:opacity-100 transition-opacity duration-500"></span>
		</a>
		...
		<nav bind:this={navContainer} class="hidden space-x-2 md:flex">
			{#each navLinks as link}
				<a
					href={link.path}
					class="nav-item relative px-4 py-2 text-sm font-medium transition-all overflow-hidden rounded-md
				...
				>
					...
					<span class="absolute inset-0 bg-[#ffde21] opacity-0 transition-opacity duration-300
						{$page.url.pathname === link.path ? 'opacity-100' : 'group-hover:opacity-100'}"></span>
						
					<!-- Active indicator dot -->
					{#if $page.url.pathname === link.path}
						<span class="absolute top-1 right-1 w-1 h-1 rounded-full bg-black"></span>
					{/if}
		...
		<button
			class="relative overflow-hidden rounded-full p-3 text-white md:hidden transition-all duration-300 border border-[#ffde21]/20"
			aria-controls="mobile-menu"
			aria-expanded={isMenuOpen}
			on:click={toggleMenu}
		>
			<span class="absolute inset-0 bg-black/30 backdrop-blur-md"></span>
			<span class="absolute inset-0 bg-[#ffde21]/10 opacity-0 transition-opacity duration-300 {isMenuOpen ? 'opacity-100' : ''}"></span>
	...
	</div>	<!-- Mobile Navigation with modern glass effect -->
	{#if isMenuOpen}
		<button
			class="fixed inset-x-0 top-24 bottom-0 z-40 backdrop-blur-xl bg-black/60 md:hidden"
		...
		>
			<div class="space-y-2">
				<!-- Decorative accent -->
				<div class="absolute top-0 right-0 h-24 w-1 bg-gradient-to-b from-[#ffde21]/80 to-transparent"></div>
				...
				{#each navLinks as link}
					...
					>
						<!-- Background element -->
						<span class="absolute inset-0 {$page.url.pathname === link.path 
							? 'bg-[#ffde21]' 
							: 'bg-black/40 group-hover:bg-[#ffde21]/20'} transition-colors duration-300"></span>
						
						<!-- Link content -->
						<span class="relative z-10 flex items-center justify-between {$page.url.pathname === link.path ? 'text-black' : 'text-white'}">
							{link.label}
				...
				<div class="mt-6 pt-6 border-t border-white/10">
					...
					<div class="flex space-x-3">
						<a href="https://github.com/" class="flex h-9 w-9 items-center justify-center rounded-full bg-black/50 text-white hover:bg-[#ffde21] hover:text-black transition-colors" aria-label="GitHub">
						...
						<a href="https://open.spotify.com/" class="flex h-9 w-9 items-center justify-center rounded-full bg-black/50 text-white hover:bg-[#ffde21] hover:text-black transition-colors" aria-label="Spotify">
							<svg class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
