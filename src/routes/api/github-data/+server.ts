...
export async function GET(event: RequestEvent) {
	...
	try {
		...
		const [reposRes, eventsRes, userRes] = await Promise.all([
			event.fetch('https://api.github.com/users/nishanajihah/repos?sort=updated&per_page=100', { headers }),
			event.fetch('https://api.github.com/users/nishanajihah/events/public?per_page=30', { headers }),
		...
		// Get pinned repos (top 3 most starred non-fork repos)
		const pinnedRepos = filteredRepos.slice(0, 3);

		// Enhance pinned repos with README images
		...
		const transformedPinnedRepos = enhancedPinnedRepos.map(repo => ({
			...
			demo_url: repo.topics.some(topic => 
				topic.includes('demo') || topic.includes('live') || topic.includes('website')
			) ? `${repo.html_url}#demo` : undefined
		}));
		...
		return new Response(JSON.stringify({ 
			...
			pinnedRepos: transformedPinnedRepos,
